...
if not TOKEN:
    raise RuntimeError("TOKEN environment variable is not set. Add your Telegram bot token as the TOKEN secret.")
...
def callback(call):
    ...
    if call.data == "settings":
        ...
        # Обновляем данные пользователей перед показом статистики
        for uid in list(users.keys()):
            try:
                chat_info = bot.get_chat(uid)

                users[uid]["username"] = chat_info.username
                users[uid]["name"] = chat_info.first_name
        ...
        text += f"Новых пользователей сегодня — {len(today_users)}\n\n"
